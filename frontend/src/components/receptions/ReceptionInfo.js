...
const ReceptionInfo = () => {
    ...
    if (!isNewReception) {
      const fetchReception = async () => {
        try {
          if (
            window.api &&
            typeof window.api.getReceptionWithLignes === "function"
          ) {
            const fetchedReception = await window.api.getReceptionWithLignes(
              id
            );
  ...
  const updateLigne = (index, field, value) => {
    ...
    if (field === "qt") {
      // parseInt peut renvoyer NaN, alors on garde 0 par défaut
  ...
  const handleSubmit = async (e) => {
    ...
    // Validation simple
    const lignesValides = lignes.filter((l) => l.article_id);
