...
const createCommandeWithLignes = async (commande, lignes) => {
    try {
        return await knex.transaction(async (trx) => {
            ...
            for (const ligne of lignes) {
                ...
                if (stockDispo < ligne.qt) {
                    throw new Error(`Stock insuffisant pour l'article ${article.designation || ligne.article_id}`);
                }

                console.log(ligne.article_id);
        ...
        console.error("❌ Erreur lors de la création de la commande avec lignes :", err);
        throw err;
    }
};
...
const updateCommandeWithLignes = async (id, commande, lignes) => {
    try {
        return await knex.transaction(async (trx) => {
            ...
            const anciennesLignes = await trx("commande_lignes").where({ commande_id: id });
        ...
        console.error("❌ Erreur lors de la mise à jour de la commande avec lignes :", err);
        throw err;
    }
};
...
const updateCommande = async (commandeData) => {
    ...
    try {
        ...
        if (!currentCommande) throw new Error(`Commande avec ID ${id} non trouvée.`);
        ...
        if (livree !== undefined && livree !== currentCommande.livree) {
            ...
            for (const ligne of lignes) {
                ...
                if (livree) {
                    ...
                    await trx("articles").where({ id: articleId })
        ...
        await trx("commandes").where({ id }).update({ ...updateData, livree });
...
const getCommandeLignes = async (commandeId) => {
    try {
        return await knex("commande_lignes").where({ commande_id: commandeId }).select("*");
    } catch (error) {
...
const updateCommandeLigne = async (ligneData) => {
    try {
        ...
        const rowsUpdated = await knex("commande_lignes").where({ id }).update(updateData);
        if (rowsUpdated === 0) throw new Error(`Ligne de commande avec ID ${id} non trouvée.`);
        return rowsUpdated;
    } catch (error) {
...
const getAllCommandesWithLignes = async () => {
    try {
        ...
        console.error("❌ Erreur lors de la récupération de toutes les commandes avec lignes :", error);
        throw error;
    }
};
